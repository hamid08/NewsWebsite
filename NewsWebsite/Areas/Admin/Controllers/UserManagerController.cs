...
namespace NewsWebsite.Areas.Admin.Controllers
{
    ...
    public class UserManagerController : BaseController
    {
        ...
        public UserManagerController(IApplicationUserManager userManager, IUnitOfWork unitOfWork, IMapper mapper, IApplicationRoleManager roleManager, IWebHostEnvironment env)
        {
        ...
        public async Task<JsonResult> GetUsers(string search, string order, int offset, int limit, string sort)
        {
            ...
            return Json(new { total, rows = allUsers });
        }
        ...
        public async Task<IActionResult> DeleteConfirmed(User model)
        {
            ...
            var user = await _uw.BaseRepository<User>().FindByIdAsync(model.Id);
            if (user == null)
                ModelState.AddModelError(string.Empty, UserNotFound);
            else
            {
                ...
                _uw.BaseRepository<User>().Update(user);
                await _uw.Commit();
                ////if (result.Succeeded)
                ////{
